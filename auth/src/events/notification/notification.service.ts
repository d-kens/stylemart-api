...
import { EmailVerificationNotification, OtpNotification, PasswordResetNotification } from '../../dtos/notification-payload';
...
export class NotificationService implements OnModuleInit {
    ...
    async sendVerificationEmail(data: EmailVerificationNotification): Promise<void> {
        try {
            await this.sendNotification(EventType.EMAIL_VERIFICATION_NOTIFICATION, data);
        } catch (error) {
            this.logger.error('Error sending verification email notification', error.stack);
            throw error;
        }
    }
    ...
    async sendPasswordReset(data: PasswordResetNotification): Promise<void> {
        try {
            ...
            this.logger.error('Error sending password reset email notification', error.stack);
            throw error;
        }
    }
}
